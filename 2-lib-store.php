...
class Storage {
  ...
  function load($name)
  {
      // (E1) GET FILE
      $this->query(
          "SELECT `file_mime`, `file_data` FROM `storage` WHERE `file_name`=?",
          [$name]
      );
      ...
      echo '<img src="data:' . $file["file_mime"] . ';base64,' . base64_encode($file["file_data"]) . '" />';
  }
